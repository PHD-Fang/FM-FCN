...
def convertCohface(dir_path, folder_list, dst_path, img_size, test_device='cuda:0'):
    '''
    处理UBFC-rppg数据集
    Args:
        dir_path:
    ...
    with tqdm(total=len(folder_list), position=0, ncols=80, desc=dir_path) as pbar:
        for folder in folder_list:
            for i in range(4):
                curr_path = dir_path + "/" + folder + "/%d/" % i 
                video_name = 'data.avi'
                label_name = 'data.hdf5'
                ...
                #   读取PPG信息
                # ---------------------------------------------------------------------#
                data = h5py.File(curr_path + label_name, "r")
                label_data = data['pulse'][:]
