...
def convertUBFCRppg(dir_path, folder_list, dst_path, img_size, test_device='cuda:0'):
    '''
    处理UBFC-rppg数据集
    Args:
        dir_path:
    ...
    with tqdm(total=len(folder_list), position=0, ncols=80, desc=dir_path) as pbar:
        for folder in folder_list:
            ...
            #   读取PPG信息
            # ---------------------------------------------------------------------#
            label_data = np.loadtxt(os.path.join(curr_path, label_name), dtype=float)[0, :].ravel()
